...
def create_search_mission(
    ...
    profile = (
        ...
        .filter(VesselTargetProfile.profile_id == target_profile_id)
        .first()
    )
    if profile is None:
    ...
    for i in range(36):
        ...
        nm_to_deg_lon = (
            1.0 / (60.0 * math.cos(math.radians(center_lat))) if center_lat != 90 else 1.0
        )
...
def finalize_mission(mission_id: int, candidate_id: int, db: Session) -> SearchMission:
    ...
    candidate = db.query(HuntCandidate).filter(HuntCandidate.candidate_id == candidate_id).first()
