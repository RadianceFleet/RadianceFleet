...
def detect_gaps_for_vessel(
    ...
    for i in range(len(points) - 1):
        ...
        try:
            from app.modules.corridor_correlator import (
                find_corridor_for_gap,
                find_dark_zone_for_gap,
            )
    ...
    if start_pt and end_pt:
        if duration_h <= 2:
            ...
            positions_json, ellipse_wkt = interpolate_hermite(
                ...
                end_pt.lat,
                end_pt.lon,
                start_sog=start_pt.sog or 0,
                start_cog=start_pt.cog or 0,
                end_sog=end_pt.sog or 0,
                end_cog=end_pt.cog or 0,
...
def detect_stale_ais_data(
    ...
    for vessel in vessels:
        ...
        for i in range(1, len(points)):
            ...
            same_heading = (
                p_prev.heading is not None
                and p_curr.heading is not None
                and p_prev.heading == p_curr.heading
            )
            same_sog = (
                p_prev.sog is not None and p_curr.sog is not None and p_prev.sog == p_curr.sog
            )
            same_cog = (
                p_prev.cog is not None and p_curr.cog is not None and p_prev.cog == p_curr.cog
            )
                        ...
                        if first_sog is not None and first_sog > 0.5:
                            ...
                            existing = (
                                ...
                                .filter(
                                    ...
                                    == points[run_start].timestamp_utc,
                                )
                                .first()
                            )
                            if not existing:
    ...
    for vessel in vessels:
        ...
        for i in range(len(points)):
            ...
            for j in range(i + 1, len(points)):
                ...
                if pi_source and pj_source and pi_source != pj_source and dt_s < 120:
                    continue
        ...
        if len(points) >= _CIRCLE_MIN_POINTS:
            for i in range(len(points)):
                ...
                if len(window) < _CIRCLE_MIN_POINTS:
                    continue
                window_hours = (
                    window[-1].timestamp_utc - window[0].timestamp_utc
                ).total_seconds() / 3600
                        ...
                        if run_hours >= 6:
                            existing = (
                                ...
                                .filter(
                                    ...
                                    == SpoofingTypeEnum.ERRATIC_NAV_STATUS,
                                    SpoofingAnomaly.start_time_utc
                                    == restricted_run[0].timestamp_utc,
                                )
                                .first()
                            )
                            if not existing:
