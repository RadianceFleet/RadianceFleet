...
class MergeOperation(Base):
    ...
    affected_records_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)

    # Relationships
    candidate: Mapped[MergeCandidate | None] = relationship("MergeCandidate")
    canonical_vessel: Mapped[Vessel] = relationship(
        "Vessel",
        foreign_keys=[canonical_vessel_id],
    )
    absorbed_vessel: Mapped[Vessel] = relationship(
        "Vessel",
        foreign_keys=[absorbed_vessel_id],
    )
