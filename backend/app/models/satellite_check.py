...
class SatelliteCheck(Base):
    ...
    provider: Mapped[str | None] = mapped_column(String(100), nullable=True, default="Sentinel-1")
    query_time_window: Mapped[str | None] = mapped_column(String(200), nullable=True)
    query_geometry: Mapped[str | None] = mapped_column(String(2000), nullable=True)
    scene_refs_json: Mapped[list | None] = mapped_column(JSON, nullable=True)
    review_status: Mapped[str] = mapped_column(
        SAEnum(SatelliteReviewStatusEnum),
        nullable=False,
        default=SatelliteReviewStatusEnum.NOT_CHECKED,
    )
    review_notes: Mapped[str | None] = mapped_column(String(2000), nullable=True)
    imagery_url: Mapped[str | None] = mapped_column(String(500), nullable=True)
    sentinel_scene_id: Mapped[str | None] = mapped_column(String(200), nullable=True)
    ...
    copernicus_url: Mapped[str | None] = mapped_column(String(500), nullable=True)
