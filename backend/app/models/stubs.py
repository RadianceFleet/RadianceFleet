...
class VesselTargetProfile(Base):
    ...
    loa_meters: Mapped[float | None] = mapped_column(Float, nullable=True)
    beam_meters: Mapped[float | None] = mapped_column(Float, nullable=True)
    typical_draft_meters: Mapped[float | None] = mapped_column(Float, nullable=True)
    funnel_color: Mapped[str | None] = mapped_column(String(50), nullable=True)
    hull_color: Mapped[str | None] = mapped_column(String(50), nullable=True)
    last_ais_position_lat: Mapped[float | None] = mapped_column(Float, nullable=True)
    last_ais_position_lon: Mapped[float | None] = mapped_column(Float, nullable=True)
    last_ais_timestamp_utc: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    profile_created_at: Mapped[datetime] = mapped_column(DateTime, default=func.now())
    created_by_analyst_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
...
class SearchMission(Base):
    ...
    analyst_id: Mapped[str | None] = mapped_column(String(100), nullable=True)
    search_ellipse_wkt: Mapped[str | None] = mapped_column(String(5000), nullable=True)
    center_lat: Mapped[float | None] = mapped_column(Float, nullable=True)
    center_lon: Mapped[float | None] = mapped_column(Float, nullable=True)
    max_radius_nm: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class HuntCandidate(Base):
    ...
    detection_time_utc: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    visual_similarity_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    length_estimate_m: Mapped[float | None] = mapped_column(Float, nullable=True)
    heading_estimate_deg: Mapped[float | None] = mapped_column(Float, nullable=True)
    ...
    satellite_scene_id: Mapped[str | None] = mapped_column(String(200), nullable=True)
    image_chip_ref: Mapped[str | None] = mapped_column(String(500), nullable=True)
    analyst_review_status: Mapped[str | None] = mapped_column(String(50), nullable=True)
...
class DarkVesselDetection(Base):
    ...
    detection_time_utc: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    length_estimate_m: Mapped[float | None] = mapped_column(Float, nullable=True)
    vessel_type_inferred: Mapped[str | None] = mapped_column(String(100), nullable=True)
