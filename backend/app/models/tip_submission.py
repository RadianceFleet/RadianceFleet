...
class TipSubmission(Base):
    ...
    submitter_email: Mapped[Optional[str]] = mapped_column(String(200), nullable=True)
...
ALLOWED_SOURCE_DOMAINS = {
    "marinetraffic.com", "vesselfinder.com", "globalfishingwatch.org",
    "fleetmon.com", "myshiptracking.com"
}
...
def validate_source_url(url: str | None) -> str | None:
    ...
    try:
        ...
        if not any(domain == d or domain.endswith("." + d) for d in ALLOWED_SOURCE_DOMAINS):
            raise ValueError(f"Source URL domain not in allowlist. Allowed: {sorted(ALLOWED_SOURCE_DOMAINS)}")
        return url
    except ValueError:
        raise
    except Exception:
