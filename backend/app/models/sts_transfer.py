...
class StsTransferEvent(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "vessel_1_id", "vessel_2_id", "start_time_utc", name="uq_sts_vessel_pair_time"
        ),
    )

    sts_id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    vessel_1_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("vessels.vessel_id"), nullable=False, index=True
    )
    vessel_2_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("vessels.vessel_id"), nullable=False, index=True
    )
    ...
    mean_proximity_meters: Mapped[float | None] = mapped_column(Float, nullable=True)
    ...
    # Analyst validation fields
    user_validated: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    ...
    corridor: Mapped[Corridor | None] = relationship("Corridor", back_populates="sts_events")
