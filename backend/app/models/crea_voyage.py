...
class CreaVoyage(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "vessel_id", "departure_port", "departure_date", name="uq_crea_voyage_dedup"
        ),
    )
    ...
    departure_port: Mapped[str | None] = mapped_column(String(255), nullable=True)
    arrival_port: Mapped[str | None] = mapped_column(String(255), nullable=True)
    commodity: Mapped[str | None] = mapped_column(String(255), nullable=True)
    cargo_volume_tonnes: Mapped[float | None] = mapped_column(Float, nullable=True)
    ...
    import_run_id: Mapped[str | None] = mapped_column(String(36), nullable=True, index=True)
