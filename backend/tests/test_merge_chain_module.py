...
class TestMergeChainsEndpoint:
    def test_endpoint_exists_in_routes(self):
        """GET /merge-chains endpoint is registered."""
        from app.api.routes import router
        paths = [r.path for r in router.routes]
        assert "/merge-chains" in paths

    @patch("app.modules.merge_chain.get_merge_chain_count")
    @patch("app.modules.merge_chain.get_merge_chains")
    def test_endpoint_returns_paginated(self, mock_get, mock_count):
        """Endpoint returns items, total, skip, limit."""
        ...
        assert data["items"][0]["chain_id"] == 1
