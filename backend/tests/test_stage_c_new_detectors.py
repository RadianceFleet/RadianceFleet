...
class TestFeatureFlags:
    def test_route_laundering_flags(self):
        ...
        assert s.ROUTE_LAUNDERING_DETECTION_ENABLED is True
        assert s.ROUTE_LAUNDERING_SCORING_ENABLED is True
    ...
    def test_pi_cycling_flags(self):
        ...
        s = Settings(_env_file=None)
        assert s.PI_CYCLING_DETECTION_ENABLED is True
        assert s.PI_CYCLING_SCORING_ENABLED is True

    def test_sparse_transmission_flags(self):
        from app.config import Settings

        s = Settings(_env_file=None)
        assert s.SPARSE_TRANSMISSION_DETECTION_ENABLED is True
        assert s.SPARSE_TRANSMISSION_SCORING_ENABLED is True

    def test_type_consistency_flags(self):
        from app.config import Settings

        s = Settings(_env_file=None)
        assert s.TYPE_CONSISTENCY_DETECTION_ENABLED is True
        assert s.TYPE_CONSISTENCY_SCORING_ENABLED is True
...
class TestRouteLaunderingDetector:
    ...
    def test_3_hop_detection(self, db):
        ...
        _make_port_call(
            db, vessel.vessel_id, int_port.port_id, arrival_utc=now - timedelta(days=20)
        )
        _make_port_call(
            db, vessel.vessel_id, sanc_port.port_id, arrival_utc=now - timedelta(days=10)
        )
        db.commit()
    ...
    def test_russian_oil_terminal_flag(self, db):
        ...
        oil_terminal = _make_port(
            db, name="Oil Terminal", country="XX", is_russian_oil_terminal=True
        )
        int_port = _make_port(db, name="Jamnagar", country="IN")

        now = datetime.utcnow()
        _make_port_call(
            db, vessel.vessel_id, oil_terminal.port_id, arrival_utc=now - timedelta(days=30)
        )
        _make_port_call(
            db, vessel.vessel_id, int_port.port_id, arrival_utc=now - timedelta(days=20)
        )
    ...
    def test_lookback_window_respected(self, db):
        ...
        _make_port_call(
            db, vessel.vessel_id, int_port.port_id, arrival_utc=now - timedelta(days=50)
        )
        db.commit()
...
class TestPICyclingDetector:
    ...
    def test_rapid_changes_detected(self, db):
        ...
        _make_history(db, vessel.vessel_id, "pi_club_name", "Gard P&I", "Skuld", days_ago=30)
        _make_history(db, vessel.vessel_id, "pi_club_name", "Skuld", "Britannia", days_ago=10)
        db.commit()
    ...
    def test_non_ig_club_higher_score(self, db):
        ...
        _make_history(db, vessel.vessel_id, "pi_club_name", "Gard P&I", "Skuld", days_ago=30)
        _make_history(db, vessel.vessel_id, "pi_club_name", "Skuld", "Unknown P&I Ltd", days_ago=10)
        db.commit()
    ...
    def test_old_changes_not_counted(self, db):
        ...
        _make_history(db, vessel.vessel_id, "pi_club_name", "Gard P&I", "Skuld", days_ago=150)
        _make_history(db, vessel.vessel_id, "pi_club_name", "Skuld", "Britannia", days_ago=120)
        db.commit()
...
class TestVesselTypeConsistencyDetector:
    ...
    def test_large_vessel_fishing_type_flagged(self, db):
        ...
        _make_vessel(db, deadweight=100000.0, vessel_type="fishing vessel")
        db.commit()
    ...
    def test_large_vessel_tanker_type_ok(self, db):
        ...
        _make_vessel(db, deadweight=100000.0, vessel_type="crude oil tanker")
        db.commit()
    ...
    def test_recent_type_change_flagged(self, db):
        ...
        _make_history(db, vessel.vessel_id, "vessel_type", "crude oil tanker", "cargo", days_ago=30)
        db.commit()
    ...
    def test_small_vessel_not_flagged(self, db):
        ...
        _make_vessel(db, deadweight=500.0, vessel_type="fishing vessel")
        db.commit()
    ...
    def test_both_signals_combined(self, db):
        ...
        _make_history(
            db, vessel.vessel_id, "vessel_type", "crude oil tanker", "pleasure craft", days_ago=15
        )
        db.commit()
