...
_OTHER_IMO_STEMS = [
    "912346", "923457", "934568", "945679", "956780", "967891",
    "978902", "989013", "910124", "921235", "932346", "943457",
    "954568", "965679", "976780", "987891", "998902", "900013",
    "911124", "922235", "933346", "944457", "955568", "966679",
    "977780", "988891", "999902", "900114", "911225", "922336",
    "933447", "944558", "955669", "966770", "977881", "988992",
    "910003", "921114", "932225", "943336", "954447", "965558",
    "976669", "987770", "998881", "900992", "911003", "922114",
    "933225", "944336", "955447", "966558", "977669", "988770",
]
...
def generate() -> list[dict]:
    ...
    for mmsi, name, vtype, flag, dwt, year in gap_vessels:
        imo = _imo(imo_idx); imo_idx += 1
        # 18 points before gap (heading east in Baltic, ~58N 20E)
        for i in range(18):
            dt = BASE_DATE + timedelta(hours=i * 1.5)
            rows.append(make_point(
                mmsi, imo, name, vtype, flag, dwt, year, "A",
                dt, 58.5 + i * 0.03, 20.0 + i * 0.15, sog=14.0, cog=95.0,
            ))
        ...
        for i in range(22):
            ...
            rows.append(make_point(
                mmsi, imo, name, vtype, flag, dwt, year, "A",
                dt, 58.9 + i * 0.02, 26.0 + i * 0.15, sog=13.5, cog=100.0,
            ))
    ...
    for mmsi, name, vtype, flag, dwt, year in loiter_vessels:
        ...
        for i in range(15):
            ...
            rows.append(make_point(
                mmsi, imo, name, vtype, flag, dwt, year, "A",
                dt, 56.5 + random.uniform(-0.001, 0.001),
                21.0 + random.uniform(-0.001, 0.001),
                sog=round(random.uniform(0.0, 0.4), 1), cog=random.uniform(0, 360),
                nav_status=1,
            ))
    ...
    sts_imo_1 = _imo(imo_idx); imo_idx += 1
    sts_imo_2 = _imo(imo_idx); imo_idx += 1
    sts_base_lat, sts_base_lon = 35.0, 23.2
    # V1 approaches
    for i in range(10):
        dt = BASE_DATE + timedelta(hours=i * 2)
        rows.append(make_point(
            sts_mmsi_1, sts_imo_1, "CASPIAN DREAM", "Chemical Tanker", "PW", 50000, 2006, "A",
            dt, 35.5 - i * 0.05, 22.5 + i * 0.07, sog=max(0.3, 10.0 - i),
        ))
    # V2 approaches
    for i in range(10):
        dt = BASE_DATE + timedelta(hours=i * 2)
        rows.append(make_point(
            sts_mmsi_2, sts_imo_2, "NEVA CARRIER", "Product Tanker", "PW", 60000, 2005, "A",
            dt, 34.5 + i * 0.05, 23.9 - i * 0.07, sog=max(0.3, 10.0 - i),
        ))
    ...
    for i in range(10):
        ...
        rows.append(make_point(
            sts_mmsi_1, sts_imo_1, "CASPIAN DREAM", "Chemical Tanker", "PW", 50000, 2006, "A",
            dt, sts_base_lat + random.uniform(-0.001, 0.001),
            sts_base_lon + random.uniform(-0.001, 0.001),
            sog=0.3, cog=180.0, nav_status=1,
        ))
        rows.append(make_point(
            sts_mmsi_2, sts_imo_2, "NEVA CARRIER", "Product Tanker", "PW", 60000, 2005, "A",
            dt, sts_base_lat + random.uniform(-0.001, 0.001),
            sts_base_lon + random.uniform(-0.001, 0.001),
            sog=0.3, cog=0.0, nav_status=1,
        ))

    # ─── 4. Circle spoof (1 vessel, 16 pts) ─────────────────────────────────
    cs_mmsi = "314320001"
    cs_imo = _imo(imo_idx); imo_idx += 1
    # 4 transit points
    for i in range(4):
        dt = BASE_DATE + timedelta(hours=i * 3)
        rows.append(make_point(
            cs_mmsi, cs_imo, "SHADOW DANCER", "Product Tanker", "CM", 75000, 1997, "A",
    ...
    for i in range(12):
        ...
        rows.append(make_point(
            cs_mmsi, cs_imo, "SHADOW DANCER", "Product Tanker", "CM", 75000, 1997, "A",
            dt, 40.8 + lat_j, 20.0 + lon_j,
            sog=4.0 + random.uniform(-0.3, 0.3), cog=random.uniform(0, 360),
        ))

    # ─── 5. Impossible position (1 vessel, 10 pts) ──────────────────────────
    ip_mmsi = "636018000"
    ip_imo = _imo(imo_idx); imo_idx += 1
    rows.append(make_point(
        ip_mmsi, ip_imo, "URSA MAJOR", "Crude Oil Tanker", "SL", 200000, 2005, "A",
    ...
    rows.append(make_point(
        ip_mmsi, ip_imo, "URSA MAJOR", "Crude Oil Tanker", "SL", 200000, 2005, "A",
        BASE_DATE + timedelta(hours=6), 53.0, 28.0, sog=14.0,
    ))
    for i in range(7):
    ...
    for i in range(20):
        ...
        rows.append(make_point(
            mp1_x_mmsi, mp1_imo, "BALTIC PHOENIX", "Crude Oil Tanker", "LR",
            150000, 2001, "A",
            dt, 58.0 + i * 0.02, 20.0 + i * 0.1, sog=14.0, cog=90.0,
        ))
    ...
    for i in range(20):
        ...
        rows.append(make_point(
            mp1_y_mmsi, mp1_imo, "BALTIC STAR", "Crude Oil Tanker", "PA",
            148000, 2008, "A",
            dt, 59.2 + i * 0.02, 22.5 + i * 0.1, sog=13.0, cog=95.0,
        ))
    ...
    for i in range(20):
        dt = BASE_DATE - timedelta(hours=120) + timedelta(hours=i * 2.4)
        rows.append(make_point(
            mp2_x_mmsi, mp2_imo, "CASPIAN TRADER", "Crude Oil Tanker", "LR",
            155000, 2003, "A",
            dt, 57.0 + i * 0.02, 19.0 + i * 0.1, sog=14.0, cog=90.0,
        ))
    ...
    for i in range(20):
        ...
        rows.append(make_point(
            mp2_y_mmsi, mp2_imo, "CASPIAN TRADER", "Crude Oil Tanker", "KM",
            153000, 2004, "A",
            dt, 57.8 + i * 0.02, 21.0 + i * 0.1, sog=13.0, cog=95.0,
        ))

    # ─── 8. Stateless MMSI (1 vessel, 15 pts) ───────────────────────────────
    # MID 199 is unallocated
    sm_mmsi = "199000001"
    sm_imo = _imo(imo_idx); imo_idx += 1
    for i in range(15):
        dt = BASE_DATE + timedelta(hours=i * 2)
        rows.append(make_point(
            sm_mmsi, sm_imo, "GHOST TANKER", "Crude Oil Tanker", "XX", 120000, 1998, "A",
            dt, 56.0 + i * 0.04, 20.0 + i * 0.2, sog=11.0,
        ))

    # ─── 9. Flag-hopping (1 vessel, 15 pts) ─────────────────────────────────
    # 3 flag changes within 90 days shown by different flag values at different times
    fh_mmsi = "374142000"
    fh_imo = _imo(imo_idx); imo_idx += 1
    ...
    for fi, flag in enumerate(fh_flags):
        for i in range(5):
            ...
            rows.append(make_point(
                fh_mmsi, fh_imo, "FLAG HOPPER", "Product Tanker", flag, 80000, 2002, "A",
                dt, 55.0 + i * 0.1, 24.0 + i * 0.3, sog=11.0,
            ))
    ...
    for mmsi, name, flag, dwt, year in clean_vessels:
        ...
        for i in range(42):
            ...
            rows.append(make_point(
                mmsi, imo, name, "Product Tanker", flag, dwt, year, "A",
    ...
    dt_mmsi = "636019000"
    dt_imo_1 = _imo(imo_idx); imo_idx += 1
    dt_imo_2 = _imo(imo_idx); imo_idx += 1
    for i in range(10):
        dt = BASE_DATE + timedelta(hours=i * 2)
        # Vessel in Baltic
        rows.append(make_point(
            dt_mmsi, dt_imo_1, "CLONE ALPHA", "Crude Oil Tanker", "LR", 160000, 2002, "A",
            dt, 58.0 + i * 0.03, 20.0 + i * 0.1, sog=13.0,
        ))
        # Same MMSI in Mediterranean (impossible to be same vessel)
        rows.append(make_point(
            dt_mmsi, dt_imo_2, "CLONE BETA", "Crude Oil Tanker", "LR", 162000, 2003, "A",
            dt, 36.0 + i * 0.02, 15.0 + i * 0.1, sog=12.0,
        ))
    ...
    for vi in range(31):
        ...
        for i in range(50):
            ...
            rows.append(make_point(
                c_mmsi, c_imo, c_name, c_vtype, c_flag, c_dwt, c_year, "A",
                dt, lat, lon, sog=round(sog, 1), cog=cog + random.uniform(-5, 5),
            ))

    # ─── 14. Draught change vessel (1 vessel, 15 pts) ───────────────────────
    dr_mmsi = "636020000"
    dr_imo = _imo(imo_idx); imo_idx += 1
    for i in range(15):
        dt = BASE_DATE + timedelta(hours=i * 3)
        # Draught drops significantly mid-track (laden -> ballast)
        rows.append(make_point(
            dr_mmsi, dr_imo, "DRAUGHT SHIFTER", "Crude Oil Tanker", "LR", 300000, 2004, "A",
            dt, 58.0 + i * 0.04, 20.0 + i * 0.2, sog=12.0,
        ))
