...
const STATUSES: AlertStatus[] = ['new', 'under_review', 'needs_satellite_check', 'documented', 'dismissed', 'confirmed_fp', 'confirmed_tp']
    ...
    <section style={card}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        {is_false_positive != null && (
          <div style={{ marginTop: 4, marginBottom: 8, padding: '8px 12px', background: 'var(--bg-base)', borderRadius: 'var(--radius)', fontSize: 13 }}>
            <div>Verdict: <strong>{is_false_positive ? 'False Positive' : 'True Positive'}</strong></div>
        ...
        {verdictMutation.error && (
          <div style={{ background: 'var(--score-critical)', color: 'white', padding: '8px 12px', borderRadius: 'var(--radius)', marginBottom: 8, fontSize: 13 }}>
            Verdict failed: {verdictMutation.error instanceof Error ? verdictMutation.error.message : 'Unknown error'}
          </div>
        )}
