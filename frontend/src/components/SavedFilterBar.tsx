...
export function SavedFilterBar({ currentFilters, onApplyFilter }: Props) {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) =>
      apiFetch(`/alerts/saved-filters/${id}`, { method: 'DELETE' }),
  ...
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', fontSize: 13 }}>
      ...
      {showSave ? (
        <form onSubmit={e => { e.preventDefault(); saveMutation.mutate({ name: saveName, filter_json: currentFilters }) }} style={{ display: 'flex', gap: 4 }}>
          <input
            value={saveName}
            onChange={e => setSaveName(e.target.value)}
            placeholder="Filter name"
            style={{ fontSize: 12, padding: '3px 6px', background: 'var(--bg-base)', color: 'var(--text-bright)', border: '1px solid var(--border)', borderRadius: 'var(--radius)' }}
          />
          <button type="submit" style={{ fontSize: 12, padding: '3px 8px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 'var(--radius)', cursor: 'pointer' }}>
            Save
          </button>
        </form>
      ) : (
        <button onClick={() => setShowSave(true)} style={{ fontSize: 12, color: 'var(--text-muted)', background: 'none', border: '1px dashed var(--border)', borderRadius: 'var(--radius)', padding: '3px 10px', cursor: 'pointer' }}>
          + Save current
        </button>
      )}
    </div>
