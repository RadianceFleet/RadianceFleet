...
export function AlertToast({ alert }: Props) {
  ...
  useEffect(() => {
    if (!alert) return;
    setCurrent(alert); // eslint-disable-line react-hooks/set-state-in-effect -- sync incoming alert prop to local state for toast display
    setVisible(true);
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{ color: band.color, fontWeight: 700, fontSize: 11, textTransform: "uppercase" }}
        >
          {band.label} Alert
        </span>
        <button
      ...
      <div>
        Score: <b style={{ color: band.color }}>{current.risk_score}</b>
      </div>
