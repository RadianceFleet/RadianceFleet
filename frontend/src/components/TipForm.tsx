...
export function TipForm({ mmsi, vesselName }: { mmsi: string; vesselName: string }) {
  ...
  if (submitted) {
    return (
      <div style={{ padding: 12, background: 'var(--bg-card)', borderRadius: 'var(--radius)', border: '1px solid var(--border)', fontSize: 13, color: 'var(--text-muted)' }}>
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} style={{ fontSize: 13, padding: '6px 14px', border: '1px solid var(--border)', borderRadius: 'var(--radius)', cursor: 'pointer', background: 'transparent', color: 'var(--text-muted)' }}>
        Flag this vessel
      </button>
    )
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError(null)
    if (!behaviorType) { setError('Please select a behavior type.'); return }
    if (detailText.length < 50) { setError('Please provide at least 50 characters of detail.'); return }
    if (detailText.length > 500) { setError('Detail text must be 500 characters or less.'); return }
    setLoading(true)
    try {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    borderRadius: 'var(--radius)', color: 'inherit', fontSize: 13,
  }

  return (
    <div style={{ padding: 16, background: 'var(--bg-card)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
      <h4 style={{ margin: '0 0 12px', fontSize: 14 }}>Flag suspicious behavior for {vesselName}</h4>
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: 10 }}>
          <select value={behaviorType} onChange={e => setBehaviorType(e.target.value)} style={inputStyle}>
        ...
        <div style={{ fontSize: 11, textAlign: 'right', marginBottom: 10,
        ...
        <div style={{ marginBottom: 10 }}>
          <input type="email" value={email} onChange={e => setEmail(e.target.value)}
            placeholder="Your email (optional -- for analyst follow-up)" style={inputStyle} />
        </div>
        ...
        {error && <p style={{ color: 'var(--score-critical)', fontSize: 13, margin: '0 0 10px' }}>{error}</p>}

        <div style={{ display: 'flex', gap: 8 }}>
          <button type="button" onClick={() => setOpen(false)}
            style={{ padding: '6px 14px', border: '1px solid var(--border)', borderRadius: 'var(--radius)', cursor: 'pointer', background: 'transparent', color: 'inherit', fontSize: 13 }}>
            Cancel
          </button>
          <button type="submit" disabled={loading}
            style={{ padding: '6px 14px', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 'var(--radius)', cursor: loading ? 'not-allowed' : 'pointer', fontSize: 13 }}>
