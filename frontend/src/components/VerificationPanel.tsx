  ...
  const handleVerify = () => {
    ...
    verifyMutation.mutate(provider, {
      onSuccess: (data) => {
        setVerifyResult(
          data.verified
            ? `Verified via ${data.provider} ($${data.cost_usd})`
            : `Not verified via ${data.provider}`
        );
      },
      onError: (err: unknown) => {
    ...
    <Card style={{ marginBottom: 16 }}>
      ...
      {budgetLoading ? (
        ...
        >
          ...
          <span
            style={{
              color: budget.remaining_usd > 0 ? "var(--score-low)" : "var(--score-critical)",
              fontWeight: 600,
            }}
          >
          ...
          <span style={{ color: "var(--text-dim)" }}>
            {" "}
            of ${(budget.budget_usd ?? budget.monthly_budget_usd ?? 0).toFixed(2)} (
            {budget.calls_this_month ?? 0} calls this month)
          </span>
        </div>
      ) : null}
      ...
      <div style={{ marginBottom: 16 }}>
        <label
          style={{ fontSize: 12, color: "var(--text-dim)", display: "block", marginBottom: 4 }}
        >
          Verify via provider
        </label>
        ...
        {verifyResult && (
          <p
            style={{
              fontSize: 12,
              color: verifyMutation.isError ? "var(--score-critical)" : "var(--score-low)",
              marginTop: 6,
            }}
          >
            {verifyResult}
