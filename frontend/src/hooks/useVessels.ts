...
export function useVesselAliases(id: string | undefined) {
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<{ vessel_id: number; aliases: VesselAlias[] }>(`/vessels/${id}/aliases`),
    enabled: !!id,
  });
}
...
export function useVesselTimeline(id: string | undefined) {
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<{ vessel_id: number; events: TimelineEvent[]; count: number }>(
        `/vessels/${id}/timeline`
      ),
    enabled: !!id,
  });
...
export function useMergeCandidates(status?: string) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      apiFetch<{ items: MergeCandidateSummary[]; total: number }>(`/merge-candidates?${params}`),
  });
}
...
export function useVesselTrack(vesselId: string | number | undefined) {
  return useQuery({
    ...
    queryFn: () => apiFetch<GeoJSON.FeatureCollection>(`/vessels/${vesselId}/track.geojson`),
    enabled: !!vesselId,
    staleTime: 60_000,
  });
}
