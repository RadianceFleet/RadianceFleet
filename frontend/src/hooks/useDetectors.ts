...
export function useVesselDetectors(vesselId: string | undefined) {
  ...
  const spoofing = useQuery({
    ...
    queryFn: () =>
      apiFetch<{ items: SpoofingAnomaly[]; total: number }>(`/spoofing/${vesselId}`).catch(
        () => null
      ),
    enabled,
    retry: false,
  ...
  const gaps = useQuery({
    ...
    queryFn: () =>
      apiFetch<{ items: AlertSummary[]; total: number }>(
        `/alerts?vessel_id=${vesselId}&limit=20&sort_by=risk_score&sort_order=desc`
      ),
    enabled,
    retry: false,
  });
  ...
  const stsEvents = useQuery({
    ...
    queryFn: () =>
      apiFetch<{ items: StsEventSummary[]; total: number }>(
        `/sts-events?vessel_id=${vesselId}&limit=20`
      ),
    enabled,
    retry: false,
  });
  ...
  const portCalls = useQuery({
    ...
    queryFn: () =>
      apiFetch<{ items: PortCallItem[]; total: number }>(`/port-calls/${vesselId}`).catch(
        () => null
      ),
    enabled,
    retry: false,
