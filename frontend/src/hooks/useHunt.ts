...
export function useCreateHuntMission() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (body: { target_profile_id: number; search_start_utc: string; search_end_utc: string }) =>
...
export function useHuntCandidates(missionId: number | undefined, filters?: { skip?: number; limit?: number }) {
  const params = buildQueryParams({ skip: filters?.skip, limit: filters?.limit })
  return useQuery({
    queryKey: ['hunt-candidates', missionId, filters],
    queryFn: () => apiFetch<{ items: HuntCandidate[]; total: number }>(`/hunt/missions/${missionId}/candidates?${params}`),
    enabled: missionId != null,
  })
}
