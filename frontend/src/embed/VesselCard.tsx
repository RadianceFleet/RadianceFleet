...
export function VesselCard({ vesselId, apiUrl }: { vesselId: number; apiUrl: string }) {
  ...
  const bandColor = data.risk_band === 'CRITICAL' ? '#dc2626' : data.risk_band === 'HIGH' ? '#ea580c' : data.risk_band === 'MEDIUM' ? '#d97706' : '#16a34a'

  return (
    <div style={cardStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <h3 style={{ margin: 0, fontSize: 14, fontWeight: 600 }}>{data.name}</h3>
        <span style={{ background: bandColor, color: '#fff', padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 700 }}>
          {data.risk_band}
        </span>
      </div>
      <div style={{ fontSize: 12, color: '#64748b', lineHeight: 1.6 }}>
        ...
        <div>Risk Score: <b>{data.risk_score}</b></div>
        {data.last_seen && <div>Last Seen: {data.last_seen.slice(0, 16).replace('T', ' ')} UTC</div>}
      </div>
