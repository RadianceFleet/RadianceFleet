...
import {
  ...
  useHuntMissions,
  useHuntCandidates,
  useAnalyzeHuntMission,
  useFinalizeHuntMission,
} from "../hooks/useHunt";
        ...
        <>
          ...
          >
            <div style={{ display: "flex", gap: 0, flex: 1 }}>
              <button style={tabStyle(tab === "missions")} onClick={() => setTab("missions")}>
                Missions
              </button>
              <button style={tabStyle(tab === "targets")} onClick={() => setTab("targets")}>
                Targets
              </button>
            </div>
            {tab === "targets" && (
...
function TargetsTab() {
  ...
  const { data, isLoading, error, refetch } = useHuntTargets({
    skip: page * PAGE_SIZE,
    limit: PAGE_SIZE,
  });
  const targets = data ?? [];
    ...
    <Card>
      ...
      {targets.length === 0 && !isLoading && !error && (
        <EmptyState
          ...
          description="Create a vessel target profile to begin hunting"
        />
      )}
...
function MissionsTab({ onSelectMission }: { onSelectMission: (id: number) => void }) {
  const [page, setPage] = useState(0);
  const { data, isLoading, error, refetch } = useHuntMissions({
    skip: page * PAGE_SIZE,
    limit: PAGE_SIZE,
  });
  const missions = data ?? [];
  ...
  return (
    <Card>
      ...
      {missions.length === 0 && !isLoading && !error && (
        <EmptyState
          title="No search missions"
          description="Create a search mission to find vessels"
        />
      )}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {missions.map((m) => (
                  ...
                  >
                    <td style={{ ...cellStyle, fontFamily: "monospace", color: "var(--accent)" }}>
                      {m.mission_id}
                    </td>
...
function MissionDetail({ missionId, onBack }: { missionId: number; onBack: () => void }) {
  ...
  const {
    ...
    error,
    refetch,
  } = useHuntCandidates(missionId, { skip: page * PAGE_SIZE, limit: PAGE_SIZE });
    ...
    <>
      ...
      <Card>
        ...
        {items.length === 0 && !isLoading && !error && (
          <EmptyState
            title="No candidates"
            description="Run analysis on this mission to generate candidates"
          />
        )}
