...
export function DarkVesselsPage() {
  ...
  const { data, isLoading, error, refetch } = useDarkVessels({
    skip: page * PAGE_SIZE,
    limit: PAGE_SIZE,
  });
          ...
          <>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {detections.map((det) => (
                    <tr key={det.detection_id} style={{ borderBottom: "1px solid var(--border)" }}>
                      <td style={{ ...cellStyle, fontFamily: "monospace" }}>{det.detection_id}</td>
                      ...
                      <td style={cellStyle}>{det.ais_match_result ?? "-"}</td>
