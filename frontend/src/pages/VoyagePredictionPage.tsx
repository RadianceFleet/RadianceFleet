...
export function VoyagePredictionPage() {
  ...
  const hasPrediction =
    prediction &&
    ((prediction.predicted_route?.length ?? 0) > 0 || (prediction.actual_route?.length ?? 0) > 0);

  const predictedPositions: LatLngExpression[] =
    prediction?.predicted_route?.map((p) => [p.lat, p.lon] as LatLngExpression) ?? [];
  const actualPositions: LatLngExpression[] =
    prediction?.actual_route?.map((p) => [p.lat, p.lon] as LatLngExpression) ?? [];
  const allPositions = [...predictedPositions, ...actualPositions];
  const bounds: LatLngBoundsExpression | undefined =
    allPositions.length >= 2 ? (allPositions as LatLngBoundsExpression) : undefined;
