  ...
  if (!editing) {
    return (
      <span
        ...
        style={{
          cursor: "pointer",
          color: tip.analyst_note ? "var(--text-bright)" : "var(--text-dim)",
        }}
        title="Click to edit"
      >
    ...
    <div style={{ maxWidth: 1200 }}>
      ...
      <Card>
        ...
        {!isLoading && tips.length === 0 && (
          <EmptyState
            title="No tips found"
            description="No tip submissions match the current filter."
          />
        )}
                        ...
                        <div style={{ display: "flex", gap: 4 }}>
                          {tip.status !== "REVIEWED" && (
                            <button
                              style={btnStyle}
                              onClick={() =>
                                updateTip.mutate({ tipId: tip.id, status: "REVIEWED" })
                              }
                            >
                              Review
                            </button>
                          ...
                          {tip.status !== "ACTIONED" && (
                            <button
                              style={{ ...btnStyle, color: "var(--score-low, #22c55e)" }}
                              onClick={() =>
                                updateTip.mutate({ tipId: tip.id, status: "ACTIONED" })
                              }
                            >
                              Action
                            </button>
                          ...
                          {tip.status !== "DISMISSED" && (
                            <button
                              style={{ ...btnStyle, color: "var(--score-critical, #ef4444)" }}
                              onClick={() =>
                                updateTip.mutate({ tipId: tip.id, status: "DISMISSED" })
                              }
                            >
                              Dismiss
                            </button>
