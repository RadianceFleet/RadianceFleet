...
export function VesselInfoSection({
  vessel,
  aliases,
  totalGaps7d,
  totalGaps30d,
}: VesselInfoSectionProps) {
    ...
    <>
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <table>
          <tbody>
            <tr>
              <td style={labelCell}>MMSI</td>
              <td style={valueCell}>{vessel.mmsi ?? "--"}</td>
            </tr>
            <tr>
              <td style={labelCell}>IMO</td>
              <td style={valueCell}>{vessel.imo ?? "--"}</td>
            </tr>
            <tr>
            ...
            <tr>
              <td style={labelCell}>Flag</td>
              <td style={valueCell}>{vessel.flag ?? "--"}</td>
            </tr>
            <tr>
              <td style={labelCell}>Vessel Type</td>
              <td style={valueCell}>{vessel.vessel_type ?? "--"}</td>
            </tr>
            <tr>
              <td style={labelCell}>Deadweight (DWT)</td>
              <td style={valueCell}>
                {vessel.deadweight != null ? vessel.deadweight.toLocaleString() : "--"}
              </td>
            </tr>
            <tr>
              <td style={labelCell}>Year Built</td>
              <td style={valueCell}>{vessel.year_built ?? "--"}</td>
            </tr>
            <tr>
              <td style={labelCell}>AIS Class</td>
              <td style={valueCell}>{vessel.ais_class ?? "--"}</td>
            </tr>
            <tr>
              <td style={labelCell}>Flag Risk Category</td>
              <td style={valueCell}>
                <span
                  style={{ color: flagRiskColors[flagRisk] ?? "var(--text-body)", fontWeight: 600 }}
                >
            ...
            <tr>
              <td style={labelCell}>MMSI First Seen</td>
              <td style={valueCell}>{formatTimestamp(vessel.mmsi_first_seen_utc)}</td>
            </tr>
            <tr>
              <td style={labelCell}>Laid Up (30d / 60d)</td>
              <td style={valueCell}>
                {vessel.vessel_laid_up_30d ? (
                  <span style={{ color: "var(--warning)" }}>Yes (30d)</span>
                ) : (
                  "No"
                )}
                {" / "}
                {vessel.vessel_laid_up_60d ? (
                  <span style={{ color: "var(--warning)" }}>Yes (60d)</span>
                ) : (
                  "No"
                )}
                {vessel.vessel_laid_up_in_sts_zone && (
                  <span style={{ color: "var(--score-critical)", marginLeft: 8 }}>
                    {" "}
                    (in STS zone)
                  </span>
                )}
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        {vessel.watchlist_entries.length === 0 ? (
          <EmptyState
            ...
            description="This vessel is not on any known watchlists."
          />
        ) : (
          <table style={tableStyle}>
            <thead>
