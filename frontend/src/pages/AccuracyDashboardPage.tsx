...
export function AccuracyDashboardPage() {
  ...
  const [signalSort, setSignalSort] = useState<{ key: SortKey; dir: SortDir }>({
    key: "lift",
    dir: "desc",
  });
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        {validation.data && !validation.data.error && (
          <>
              ...
              {[
                ...
                <div key={m.label} style={metricBox}>
                  <div style={metricValue}>
                    {typeof m.value === "number" && m.value < 10 ? m.value.toFixed(3) : m.value}
                  </div>
            ...
            <h4 style={{ fontSize: 12, color: "var(--text-muted)", margin: "0 0 8px" }}>
              Confusion Matrix
            </h4>
            <table style={{ borderCollapse: "collapse", marginBottom: 8 }}>
              ...
              <tbody>
                <tr style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td style={{ ...cellStyle, color: "var(--accent)", fontWeight: 700 }}>
                    {validation.data.confusion_matrix.tp}
                  </td>
                  <td style={{ ...cellStyle, color: "var(--score-critical)" }}>
                    {validation.data.confusion_matrix.fn}
                  </td>
                </tr>
                <tr style={{ borderBottom: "1px solid var(--border)" }}>
                  <td style={{ ...cellStyle, fontWeight: 600 }}>Actual -</td>
                  <td style={{ ...cellStyle, color: "var(--warning)" }}>
                    {validation.data.confusion_matrix.fp}
                  </td>
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        {sweep.error && (
          <ErrorMessage error={sweep.error} subject="sweep data" onRetry={refetchSweep} />
        )}
        {!sweep.error && sweep.data && sweep.data.length === 0 && (
          <EmptyState
            title="No sweep data"
            description="Requires ground truth labels to generate sweep analysis."
          />
        )}
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        {signals.error && (
          <ErrorMessage error={signals.error} subject="signals" onRetry={refetchSignals} />
        )}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "var(--bg-base)" }}>
                  <th style={headStyle} onClick={() => handleSignalSort("signal")}>
                    Signal
                  </th>
                  <th style={headStyle} onClick={() => handleSignalSort("tp_freq")}>
                    TP Freq
                  </th>
                  <th style={headStyle} onClick={() => handleSignalSort("fp_freq")}>
                    FP Freq
                  </th>
                  <th style={headStyle} onClick={() => handleSignalSort("lift")}>
                    Lift
                  </th>
                </tr>
              </thead>
        ...
        {signals.data && signals.data.length === 0 && (
          <p style={{ color: "var(--text-muted)", fontSize: 13 }}>
            No signal data (need both TP and FP predictions).
          </p>
        )}
      </Card>
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        {liveSignals.error && (
          <ErrorMessage
            error={liveSignals.error}
            subject="live signals"
            onRetry={refetchLiveSignals}
          />
        )}
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        {analyst.error && (
          <ErrorMessage error={analyst.error} subject="analyst metrics" onRetry={refetchAnalyst} />
        )}
        {analyst.data && (
          <>
            <div style={{ display: "flex", gap: 16, marginBottom: 12, flexWrap: "wrap" }}>
              ...
              <div style={metricBox}>
                <div style={{ ...metricValue, color: "var(--score-critical)" }}>
                  {analyst.data.confirmed_fp}
                </div>
              ...
              <div style={metricBox}>
                <div
                  style={{
                    ...metricValue,
                    color: analyst.data.fp_rate > 0.3 ? "var(--score-critical)" : "var(--accent)",
                  }}
                >
              ...
              {correlationData.slice(0, 20).map((row) => (
                ...
                >
                  ...
                  <td
                    style={{
                      ...cellStyle,
                      color: row.fp_rate > 0.3 ? "var(--danger)" : "var(--text)",
                    }}
                  >
      ...
      {!correlation.error && correlationData && correlationData.length === 0 && (
        <EmptyState
          ...
          description="Requires analyst-reviewed alerts with multiple signals."
        />
      )}
    </div>
  );
