...
export function FleetAnalysisPage() {
  ...
  const { data: clusterDetail, isLoading: detailLoading } =
    useFleetClusterDetail(selectedClusterId);
    ...
    <div style={{ maxWidth: 1100 }}>
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        {alertsUnavailable && (
          <EmptyState
            ...
            description="Fleet alerts endpoint is not yet available. Run fleet pattern detection first."
          />
        )}
        {!alertsLoading && !alertsUnavailable && alerts.length === 0 && (
          <EmptyState
            title="No fleet alerts"
            description="No fleet-level alerts have been generated yet."
          />
        )}
        {alerts.length > 0 && (
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        {clustersUnavailable && (
          <EmptyState
            title="Data not available"
            description="Owner clusters endpoint is not yet available. Run owner deduplication first."
          />
        )}
        {!clustersLoading && !clustersUnavailable && clusters.length === 0 && (
          <EmptyState
            title="No owner clusters"
            description="Run owner deduplication to generate clusters."
          />
        )}
        {clusters.length > 0 && (
          <div style={{ overflowX: "auto" }}>
      ...
      {selectedClusterId != null && (
        <Card>
          ...
          >
            <h3 style={{ ...sectionHead, margin: 0 }}>Cluster #{selectedClusterId} Detail</h3>
            <button
          ...
          {clusterDetail && (
            <>
              <div style={{ fontSize: 13, marginBottom: 12 }}>
                ...
                {clusterDetail.country && (
                  <span style={{ color: "var(--text-dim)" }}> ({clusterDetail.country})</span>
                )}
