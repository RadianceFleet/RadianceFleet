...
export function DetectionPanel() {
  ...
  const runFullPipeline = async () => {
    ...
    const steps = [
      {
        name: "Gap detection",
        fn: () => gapDetect.mutateAsync(Object.keys(dateBody).length ? dateBody : undefined),
      },
      {
        name: "Spoofing detection",
        fn: () => spoofDetect.mutateAsync(Object.keys(dateBody).length ? dateBody : undefined),
      },
      {
        name: "Loitering detection",
        fn: () => loiterDetect.mutateAsync(Object.keys(dateBody).length ? dateBody : undefined),
      },
      {
        name: "STS detection",
        fn: () => stsDetect.mutateAsync(Object.keys(dateBody).length ? dateBody : undefined),
      },
    ...
    for (const step of steps) {
      ...
      try {
        ...
        setPipelineError(
          `${step.name} failed: ${err instanceof Error ? err.message : String(err)}`
        );
    ...
    <div style={{ maxWidth: 700 }}>
      ...
      <Card style={{ marginBottom: "1rem" }}>
        ...
        <DetectionButton
          label="Detect Gaps"
          mutation={gapDetect}
          body={Object.keys(dateBody).length ? dateBody : undefined}
        />
        <DetectionButton
          label="Detect Spoofing"
          mutation={spoofDetect}
          body={Object.keys(dateBody).length ? dateBody : undefined}
        />
        <DetectionButton
          label="Detect Loitering"
          mutation={loiterDetect}
          body={Object.keys(dateBody).length ? dateBody : undefined}
        />
        <DetectionButton
          label="Detect STS"
