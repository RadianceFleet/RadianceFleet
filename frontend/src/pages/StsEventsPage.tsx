  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      ...
      >
        ...
        <button style={tabStyle(tab === "chains")} onClick={() => setTab("chains")}>
          Chains
        </button>
      </div>
  ...
  return (
    <Card>
      ...
      {chains.length === 0 && !isLoading && !error && (
        <EmptyState
          title="No STS relay chains"
          description="Relay chain alerts will appear here once detected"
        />
      )}
      {chains.length > 0 && (
        <>
...
function EventsTab() {
  const [page, setPage] = useState(0);
  const { data, isLoading, error, refetch } = useStsEvents({
    skip: page * PAGE_SIZE,
    limit: PAGE_SIZE,
  });
              ...
              <tbody>
                {events.map((evt) => (
                  ...
                  >
                    ...
                    <td style={cellStyle}>{(evt.detection_type as string) ?? "-"}</td>
                    <td style={{ ...cellStyle, whiteSpace: "nowrap" }}>
                      {formatTimestamp(evt.start_time_utc as string | null)}
                    </td>
                    <td style={{ ...cellStyle, textAlign: "right" }}>
                      {evt.duration_minutes != null ? (evt.duration_minutes as number) : "-"}
                    </td>
                    <td style={{ ...cellStyle, textAlign: "right" }}>
                    ...
                    <td style={cellStyle}>
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        ...
                        <button
                          title="Confirm"
                          onClick={() =>
                            validation.mutate({ stsId: evt.sts_id, user_validated: true })
                          }
                          style={{
                        ...
                        <button
                          title="Reject"
                          onClick={() =>
                            validation.mutate({ stsId: evt.sts_id, user_validated: false })
                          }
                          style={{
                            border: "1px solid var(--border)",
                            borderRadius: "var(--radius)",
