...
export function DetectorResultsPage() {
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
              ...
              <tbody>
                {spoofingList.map((s, i) => (
                  <tr key={s.anomaly_id ?? i} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={cellStyle}><ScoreBadge score={s.risk_score_component} size="sm" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <tbody>
              {gapItems.map((a: any) => (
                <tr key={a.gap_event_id} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={cellStyle}><ScoreBadge score={a.risk_score} size="sm" /></td>
                  <td style={cellStyle}>{formatTimestamp(a.gap_start_utc)}</td>
                  <td style={cellStyle}>{a.duration_minutes != null ? `${(a.duration_minutes / 60).toFixed(1)}h` : '-'}</td>
              ...
              <tbody>
                {loiteringList.map((l, i) => (
                  <tr key={l.loiter_id ?? i} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={cellStyle}>{l.duration_hours != null ? `${l.duration_hours.toFixed(1)}h` : '-'}</td>
                    <td style={cellStyle}>
                      {l.corridor_id != null
      ...
      <CollapsibleSection title="Flag & Identity History" count={flagHistoryItems.length} defaultOpen={false}>
            ...
            <tbody>
              {flagHistoryItems.map((h: any, i: number) => (
                <tr key={h.vessel_history_id ?? i} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ ...cellStyle, color: 'var(--score-medium)', textDecoration: 'line-through' }}>
                    {h.old_value || '(none)'}
                  </td>
      ...
      <CollapsibleSection title="Port Calls" count={portCallItems.length} defaultOpen={false}>
        ...
        {!portCalls.isLoading && portCallItems.length === 0 && (
          <EmptyState title="No port calls available" description="Port call data may not be available for this vessel." />
        )}
